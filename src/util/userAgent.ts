...
export const generateUserAgent = (): [string, string] => {
	...
	userAgent = userAgent.replaceAll('{version}', String(version));
	...
	const secChUa = (isEdge ? secChUaEdge : secChUaChrome).replaceAll('{version}', String(version));

	switch (platform) {
		case Platforms.Mac:
			return [userAgent.replace('{platform}', platformMac), secChUa];
		case Platforms.Linux:
			return [userAgent.replace('{platform}', platformLinux), secChUa];
		case Platforms.Android:
			userAgent = isEdge ? edgeMobileUA : chromeMobileUA;
			return [userAgent.replace('{platform}', platformAndroid).replace('{version}', String(version)), secChUa];
		default:
			return [userAgent.replace('{platform}', platformWindows), secChUa];
	}
};
